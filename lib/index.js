...
function registerGitHttp(ctx, git, fs, editors = new ExternalOpen(fs), term = new TerminalHub()) {
	...
	const handler = async (req, res) => {
		...
		let result;
		try {
			if (method === "GET" && route === "/git/probe") result = await wrap(() => git.probe(rootOf()));
			else if (method === "GET" && route === "/git/identity") result = await wrap(() => git.identity(rootOf()));
			else if (method === "POST" && route === "/git/init") {
				const body = await readJson(req);
				const name = typeof body.name === "string" ? body.name : "";
				const email = typeof body.email === "string" ? body.email : "";
				const branch = typeof body.branch === "string" ? body.branch : "";
				result = await wrap(() => git.initRepo(rootOf(body), {
					name,
					email,
					branch
				}));
			} else if (method === "GET" && route === "/git/status") result = await wrap(() => git.status(rootOf()));
			...
			} else if (method === "GET" && route === "/git/branches") result = await wrap(() => git.branches(rootOf()));
				...
				const body = await readJson(req);
				result = await wrap(async () => {
					return { message: await generateCommitMessage(ctx, git, rootOf(body), { template: typeof body.template === "string" ? body.template : void 0 }) };
				});
			} else if (method === "POST" && route === "/git/push") {
				const body = await readJson(req);
				result = await wrap(() => git.push(rootOf(body), void 0, parsePushMode(body.pushMode)));
			} else if (method === "POST" && route === "/git/pull") {
				const body = await readJson(req);
				result = await wrap(() => git.pull(rootOf(body), void 0, parsePullMode(body.pullMode)));
				...
				else result = await wrap(() => git.commitFiles(rootOf(), hash));
			} else if (method === "GET" && route === "/git/commit-diff") {
				const hash = query(url, "hash");
				const path = query(url, "path");
				if (hash === void 0 || path === void 0) result = fail("BAD_REQUEST");
				else result = await wrap(() => git.commitDiff(rootOf(), hash, path));
