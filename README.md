...
| Version | **0.1.36** (npm tag `latest`) |
| Registry | https://registry.npmjs.org |

```
+ dsh-workbench-plugin@0.1.36
```
...
1. Install the plugin (pin the version; do not omit `@0.1.36`):

```bash
dsh plugin --profile web add dsh-workbench-plugin@0.1.36
```

`dsh plugin add` is implemented with pnpm. pnpm 11 waits **24 hours** after a version is published before it will pick it as `latest`. A bare `dsh-workbench-plugin` (no `@version`) can therefore install **0.1.0** and still exit 0. Pinning `@0.1.36` requests that release explicitly.
...
**Version 0.1.1 does not include the upgrade checker and will not display the notice.** Install 0.1.36 manually using the command above. Later releases will prompt in the UI.
