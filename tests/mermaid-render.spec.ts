...
// jsdom has no SVG text measurement; dagre-d3 layout needs getBBox.
...
describe('renderMermaidBlocks', () => {
  it('renders a flowchart code block into an svg', async () => {
    ...
    const found = await renderMermaidBlocks(root, classes, 'fail', () => false)
    expect(found).toBe(1)
  ...
  it('honours cancellation mid-render', async () => {
    const root = document.createElement('div')
    root.innerHTML = '<pre><code class="language-mermaid">graph TD\n  A-->B</code></pre>'
    const found = await renderMermaidBlocks(root, classes, 'fail', () => true)
    expect(found).toBe(0)
    // Cancelled before processing: original pre block still there (replaced only after check).
    expect(root.querySelectorAll('pre').length).toBe(1)
  })
})
