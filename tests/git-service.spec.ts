...
describe('GitService', () => {
  ...
  it('includes remote-only commits and origin/* marks in the graph log', async () => {
    ...
    await writeFile(join(a, 'a.txt'), 'seed\n')
    ...
    await writeFile(join(a, 'a.txt'), 'local\n')
    await git.stage(a, ['a.txt'])
    await git.commit(a, 'local only')
    await writeFile(join(b, 'a.txt'), 'remote\n')
    await git.stage(b, ['a.txt'])
  ...
  it('refuses push after fetching when the remote grew', async () => {
    ...
    await git.stage(a, ['a.txt'])
    await git.commit(a, 'on a')
    await git.push(a)
    await writeFile(join(b, 'b.txt'), 'from-b\n')
    await git.stage(b, ['b.txt'])
    await git.commit(b, 'on b')
    expect((await git.status(b)).probe.behind).toBe(0)
  ...
  it('merges a diverged branch with the default pull mode', async () => {
    const parent = await tempDir('dsh-diverge-merge-')
    const remote = join(parent, 'remote.git')
    const a = join(parent, 'a')
    await mkdir(remote)
    await mkdir(a)
    ...
    await git.stage(b, ['b.txt'])
    await git.commit(b, 'on b')
    await expect(git.pull(b, undefined, 'ff-only')).rejects.toMatchObject({ code: 'DIVERGED' })
    const pulled = await git.pull(b, undefined, 'merge')
    expect(pulled.remote).toBe('origin')
    expect((await git.status(b)).probe.behind).toBe(0)
    expect(await readFile(join(b, 'a.txt'), 'utf8')).toBe('from-a\n')
    expect(await readFile(join(b, 'b.txt'), 'utf8')).toBe('from-b\n')
  })
})
