...
export class ControlPlaneService {
  ...
  async snapshot(sessionId: string | undefined): Promise<ControlPlaneSnapshot> {
    ...
    if (ambientNode !== undefined) {
      ambientNode.badge = String(seen.size)
      ambientNode.stats = [
        { label: '插件总数', value: String(seen.size) },
        { label: '已启用', value: String(enabledCount) },
        { label: '已禁用', value: String(seen.size - enabledCount) },
      ]
    }
  ...
  private async buildAgentBranch(
    ...
    const effectiveModel = knobs.modelOverride !== null
      ...
      : modelLine
    const messages = messageCount(absent ? undefined : agent)
    ...
    nodes.push({
      ...
      badge: children.length > 0 ? `${children.length} 子 Agent` : undefined,
      adjustable: false,
      adjustKind: 'none',
      agentId: agent.id,
      current,
      ...
      stats: [
        ...
        { label: '工具', value: `${schemas.length} 个可见${deniedTools > 0 ? ` · ${deniedTools} 个被禁用` : ''}` },
        { label: 'Prompt 段', value: `${sections.length} 段${knobs.promptAppend.trim() !== '' ? ' · 已追加控制面片段' : ''}` },
        { label: '会话日志', value: `${messages} 条` },
        { label: '子 Agent', value: String(children.length) },
        ...knobs.preStepReject ? [{ label: '门禁', value: '已开启（拒绝步骤）' }] : [],
      ],
      lockReasonZh: isSub
    ...
    nodes.push({
      ...
      badge: knobs.modelOverride !== null
        ? knobs.modelOverride.model
        : (agent.options?.model ?? '默认'),
      adjustable: true,
      adjustKind: 'model',
      agentId: agent.id,
      stats: [
        { label: '默认模型', value: modelLine },
        { label: '生效模型', value: effectiveModel },
        ...knobs.modelOverride !== null
          ? [{ label: '覆盖来源', value: `${knobs.modelOverride.provider} / ${knobs.modelOverride.model}` }]
          : [],
      ],
    })
    ...
    nodes.push({
      ...
      agentId: agent.id,
      stats: [
        { label: '可见工具', value: String(schemas.length) },
        { label: '已禁用', value: String(deniedTools) },
        { label: '可用', value: String(schemas.length - deniedTools) },
      ],
    })
    for (const schema of schemas) {
      ...
      nodes.push({
        ...
        detail: denied
          ? '已禁用'
          : (desc.length > 46 ? `${desc.slice(0, 46)}…` : desc) || '无描述',
    ...
    nodes.push({
      ...
      agentId: agent.id,
      stats: [
        { label: '提示词段', value: String(sections.length) },
        { label: '已追加片段', value: knobs.promptAppend.trim() !== '' ? '是' : '否' },
        { label: 'complete 保护', value: sections.some(s => s.complete === true) ? '有（不可替换）' : '无' },
      ],
    })
    for (const section of sections.slice(0, 200)) {
      ...
      nodes.push({
        ...
        agentId: agent.id,
        stats: [
          { label: '长度', value: `${text.length} 字符` },
          { label: '类型', value: complete ? 'complete（受保护）' : '普通段' },
        ],
    ...
    nodes.push({
      ...
      stats: [
        { label: '会话日志', value: `${messages} 条` },
        { label: 'Memory 服务', value: '无（日志 + workspace）' },
      ],
    ...
    nodes.push({
      ...
      stats: [
        { label: '子 Agent', value: String(children.length) },
        { label: '门禁', value: knobs.preStepReject ? '已开启（拒绝步骤）' : '关闭（正常执行）' },
        { label: '能力', value: 'followup / steer / inject' },
      ],
    })
