 ...
 * Each agent/subagent cluster also emits a dashed “capability zone” band that
 * the canvas draws behind its fan so the diagram reads like a real architecture
 * control plane with nested containers.
 */
...
export const AGENT_W = 196
export const AGENT_H = 66
/** Agent right edge → vertical fork rail. */
export const STEM = 44
/** Vertical rail → child left edge. */
export const BRANCH = 52
export const V_GAP = 22
export const CLUSTER_GAP = 84
export const PAD = 48
export const JUNCTION_R = 4.5
/** Padding around a cluster's boxes for its dashed zone band. */
...
export interface TopoZone {
  id: string
  /** Zone root node kind — 'agent' (root spine) or 'subagent'. */
  kind: 'agent' | 'subagent'
  /** Root node label shown in the zone corner chip. */
  label: string
  colorIndex: number
  x: number
  y: number
...
export interface TopoLayout {
  ...
  /** Dashed container bands drawn behind the fan of each agent cluster. */
  zones: TopoZone[]
  width: number
  height: number
}
...
interface Cluster {
  ...
  height: number
  rootId: string
  /** Root node label for the zone chip. */
  rootLabel: string
  /** Root node kind — 'agent' or 'subagent'. */
  rootKind: 'agent' | 'subagent'
  rootColor: number
...
function layoutAgent(
  ...
  if (fan.length === 0) {
    return {
      boxes, junctions, edges, width: a.w, height, rootId: agent.id,
      rootLabel: agent.label, rootKind: agent.kind === 'subagent' ? 'subagent' : 'agent',
      rootColor: stemColor, rootCy: agentCy,
    }
  }
  ...
  return {
    boxes, junctions, edges, width, height, rootId: agent.id,
    rootLabel: agent.label, rootKind: agent.kind === 'subagent' ? 'subagent' : 'agent',
    rootColor: stemColor, rootCy: agentCy,
  }
}
...
/** Bounding box of a placed cluster, expanded for its dashed zone band. */
function zoneBounds(
  boxes: TopoNodeBox[],
  junctions: TopoJunction[],
  pad: number,
...
export function layoutControlPlaneTopology(nodes: readonly ControlPlaneNode[]): TopoLayout {
  ...
  for (const cluster of clusters) {
    ...
    const bounds = zoneBounds(cluster.boxes, cluster.junctions, ZONE_PAD)
    zones.push({
      id: `zone:${cluster.rootId}`,
      kind: cluster.rootKind,
      label: cluster.rootLabel,
      colorIndex: cluster.rootColor,
      x: bounds.x + dx,
      y: bounds.y + dy,
      w: bounds.w,
      h: bounds.h,
  ...
  return {
    ...
    junctions,
    zones,
    width: Math.max(maxRight + PAD, 680),
